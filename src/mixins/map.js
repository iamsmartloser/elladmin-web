...
export default {
  ...
  methods: {
    ...
    drawCircle(point, radius, circleOptions) {
      ...
      const circle = new BMap.Circle(new BMap.Point(point.lng, point.lat), r, option)

      this.map.addOverlay(circle)
      this.radiusLabel = new BMap.Label('半径:' + r, {})
      console.log('vertexMarkers:', circle)
      console.log('circle:', circle.vertexMarkers['1'])
      // if (circle.vertexMarkers[0].setLabel) {
      //   circle.vertexMarkers[0].setLabel(this.radiusLabel)
      // }
      // // 如果允许编辑
      // if (option.enableEditing) {
      //   circle.addEventListener('lineupdate', (e) => {
      //     console.log('lineupdate:', e.overlay)
      //     console.log('lineupdate latLng:', e.overlay)
      //     console.log('lineupdate radius:', e.overlay.radius)
      //     if (!this.radiusLabel) {
      //       this.radiusLabel = new BMap.Label('半径:' + e.overlay.radius, {})
      //     } else {
      //       this.map.removeOverlay(this.radiusLabel)
      //       this.radiusLabel = new BMap.Label('半径:' + e.overlay.radius, {})
      //     }
      //     e.overlay.vertexMarkers[1].setLabel(this.radiusLabel)
      //
      //     //
      //   })
      // }
